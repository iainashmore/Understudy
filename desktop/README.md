...
`build/icon.png` and `build/icon.ico`, generated by `build/make_icon.py` --
two overlapping window shapes, the understudy standing behind the principal.
Regenerate with `python3 desktop/build/make_icon.py` from the repository root.
It is drawn rather than drafted because the only size that matters is 16px in
the taskbar, and that is easier to iterate on in code than in an editor.
