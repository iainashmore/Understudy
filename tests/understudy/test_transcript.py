...
def test_the_product_is_named_with_a_capital(tmp_path):
    """Understudy in prose, `understudy` only where it is literally the
    command, the package or the repository. A footer is prose."""
    run = make_run(tmp_path, [result()])
    assert "Generated by Understudy" in render_markdown(run)
    assert "Generated by Understudy" in one(run)
