...
from flowrunner.drivers.web import WebDriver, _AnchorTarget
from flowrunner.flow import Target
from flowrunner.keyboard import TypingStyle
from flowrunner.os_pointer import ViewportOrigin, origin_from_window
...
def test_read_origin_survives_a_page_that_will_not_answer():
    ...
    from flowrunner.os_pointer import read_origin
...
def cursor_exists(monkeypatch):
    monkeypatch.setattr("flowrunner.os_pointer.available", lambda: True)
    monkeypatch.setattr("flowrunner.os_pointer.unavailable_reason", lambda: None)
...
def test_a_machine_without_a_cursor_says_so(monkeypatch):
    monkeypatch.setattr("flowrunner.os_pointer.available", lambda: False)
    monkeypatch.setattr("flowrunner.os_pointer.unavailable_reason",
...
def os_driver(monkeypatch):
    ...
    monkeypatch.setattr("flowrunner.drivers.web.set_cursor",
                        lambda x, y: cursor["moved"].append((x, y)))
    monkeypatch.setattr("flowrunner.drivers.web.click_at",
