...
def _tags_markdown(subject) -> str:
...
def _image(run_dir: Path, relative: str, width: int, embed: bool,
           base: str = "") -> str:
    """`relative` is always relative to the run directory, because that is how
    the runner records it. `base` is the folder the file being written lives
    in, when that is not the run directory -- a prompt run's own transcript
    sits inside its folder, so its images are beside it rather than under it.
    """
...
def _beside(relative: str, base: str) -> str:
    """A run-relative path, rewritten for a file inside `base`."""
...
def render_markdown(
    ...
    if subject.recorded:
        out.append("- **Under test** " + _tags_markdown(subject))
        if subject.notes:
            out.append(f"- **{SUBJECT_LABELS['notes']}** {subject.notes}")
    out += [
    ...
    # A front door, not a compendium. Each prompt run has its own transcript
    # in its own folder: twelve prompts is twelve files, not one page a reader
    # scrolls through hunting for the third.
    out += ["", "## Prompt runs", ""]
    for result in results:
        folder = folder_of(result)
        mark = STATUS_MARK.get(result.get("status"), result.get("status"))
        label = result["prompt_id"]
        if result.get("repeat_index"):
            label += f" (repeat {result['repeat_index'] + 1})"
        line = (f"- [{label}]({folder}/transcript.md) — {mark}"
                f" · {result.get('duration_ms', 0)} ms")
        if result.get("error"):
            line += f" · {_escape(result['error'])[:80]}"
        out.append(line)
...
def folder_of(result: dict[str, Any]) -> str:
    """The directory this prompt run wrote into.

    Taken from what it actually produced rather than rebuilt from the id and
    the repeat index: the runner decides the name, and two places computing it
    separately is two places to get it wrong the day the rule changes.
    """
    for relative in (result.get("screenshots") or []):
        head = str(relative).replace("\\", "/").split("/")[0]
        if head:
            return head
    recording = result.get("recording") or ""
    head = str(recording).replace("\\", "/").split("/")[0]
    return head or _slug(result)
...
def _recordings_markdown(results: list[dict[str, Any]], base: str = "") -> list[str]:
    ...
    for result in videos:
        ...
        source = _beside(result["recording"], base)
        out += [
            f'<video controls width="460" src="{source}"></video>', "",
            f"[{source}]({source})", "",
        ]
...
def _variant_section(run_dir: Path, result: dict[str, Any], embed: bool,
                     ...
                     heading: bool = True, base: str = "") -> list[str]:
    out: list[str] = []
    if heading:
    ...
    for name, relative in (result.get("read_images") or {}).items():
        if name not in shown_reads:
            out += ["", f"Recorded `{name}` region:", "",
                    _image(run_dir, relative, RESPONSE_IMAGE_WIDTH, embed, base)]

    out += _timeline_markdown(run_dir, result, embed, narration or {}, base)
...
def _timeline_markdown(run_dir: Path, result: dict[str, Any], embed: bool,
                       narration: dict[str, str], base: str = "") -> list[str]:
    ...
    for entry in entries:
        ...
        for relative in entry.screenshots:
            out += [_image(run_dir, relative, THUMBNAIL_WIDTH, embed, base), "",
    ...
    if orphans:
        ...
        for relative in orphans:
            out += [f"**{_label_of(relative)}**", "",
                    _image(run_dir, relative, THUMBNAIL_WIDTH, embed, base), ""]
    return out


def render_one(run_dir: Path | str, result: dict[str, Any],
               embed: bool = False) -> str:
    """One prompt run's transcript, written to stand alone.

    Paths inside it are relative to the prompt run's own folder, because that
    is where the file lives -- a sweep of twelve prompts is twelve of these,
    not one page a reader scrolls through looking for the third.
    """
    run_dir = Path(run_dir)
    folder = folder_of(result)
    ...
    if subject.recorded:
        out.append("- **Under test** " + _tags_markdown(subject))
    out += [
        f"- **Run** `{run_dir.name}` · {result.get('backend', '')}",
        f"- **Status** {STATUS_MARK.get(result.get('status'), result.get('status'))}"
        f" · {result.get('duration_ms', 0)} ms",
        f"- **Started** {result.get('timestamp', '?')}",
    ]
    for label, names in (("Input", ("flow.yaml", "prompts.yaml", "prompts.csv")),
                         ("Output", ("results.jsonl", "results.csv"))):
        found = [n for n in names if (run_dir / n).exists()]
        if found:
            out.append(f"- **{label}** "
                       + ", ".join(f"[`{n}`](../{n})" for n in found))

    out += _recordings_markdown([result], base=folder)
    out += _variant_section(run_dir, result, embed, narration, heading=False,
                            base=folder)
    out += ["", "---", "",
            "_Generated by understudy. Screenshots are the run's own captures; "
            "response text is whatever the flow's `read` step extracted._"]
...
def write_transcript(
    ...
    """The run's index, and one transcript per prompt run beside it."""
    run_dir = Path(run_dir)
    ...
    for result in results:
        ...
        (folder / filename).write_text(
            render_one(run_dir, result, embed=embed), encoding="utf-8")
    path = run_dir / filename
    path.write_text(render_markdown(run_dir, results, embed=embed),
                    encoding="utf-8")
    return path
