...
MARKS = {
    "asked": ("?!", "the question changed between these runs"),
    "same": ("", "unchanged"),
...
def render_markdown(comparison: Comparison) -> str:
    ...
    for row in _ordered(comparison):
        ...
        asked = ("_(the question changed — see below)_" if row.asked_differently
                 else _escape(row.prompt))
        out.append(f"| {mark} | **{_escape(row.prompt_id)}**<br>{asked} | {cells} |")
    ...
    if changed:
        ...
        for row in changed:
            out += [f"### {row.prompt_id}", ""]
            if row.asked_differently:
                # Say it before the answers, not after: whatever they did, they
                # answered different questions, and a reader who reads the
                # replies first has already drawn the wrong conclusion.
                out += ["**The flow changed between these runs — this prompt "
                        "is not the same prompt.** Whatever the answers did "
                        "below, they are answers to different questions.", ""]
                for column, asked in zip(comparison.columns, row.prompts):
                    out += [f"**{column.heading}** asked", "",
                            "```text", (asked if asked is not None
                                        else "(this run has no result for it)"),
                            "```", ""]
            else:
                out += [f"> {_escape(row.prompt)}", ""]
    ...
    out += ["", "---", "",
            ...
            "answer did not; `!` means it moved; `?!` means the two runs were "
            "not asked the same question._"]
    return "\n".join(out) + "\n"
...
def render_html(comparison: Comparison, out_dir: Path | None = None) -> str:
    ...
    for row in _ordered(comparison):
        ...
        rows.append(
            ...
            f'<td><b>{_e(row.prompt_id)}</b><div class="lede">'
            + ("the question changed" if row.asked_differently else _e(row.prompt))
            + "</div></td>"
            f"{cells}</tr>"
        )
...
/* Not red: nothing is wrong with the software. The comparison is void,
   which is a different thing and should not look like a regression. */
tr.asked td.mark { color:var(--warn); }
tr.asked { background:rgba(224,169,79,.10); }
...
def _ordered(comparison: Comparison) -> list[Row]:
    ...
    rank = {"asked": 0, "changed": 1, "failed": 2, "missing": 3,
            "reworded": 4, "same": 5}
